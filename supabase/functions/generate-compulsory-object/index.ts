...
function buildPrompt(fields: Record<string, string>): string {
  ...
  return `Create a photorealistic image of a single historical object on a plain white background. Studio lighting, sharp focus, high detail.
...
- Shown clearly on a plain white background with no other objects
- Historically accurate in material, texture, wear, and construction
- Appropriately scaled and well-lit
- No people, no hands, no scenery — just the object itself
...
serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: "Object generation failed" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    const storedUrl = await saveToStorage(generatedImageUrl, "compulsory-object", className);
    ...
    console.error("generate-compulsory-object error:", e);
    return new Response(
